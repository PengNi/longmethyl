...
def combine_fb_of_freqtxt(report_fp):  # ont/ccs freq
    pos2info = {}
    poses = set()
    if report_fp.endswith(".gz"):
        infile = gzip.open(report_fp, 'rt')
    else:
        infile = open(report_fp, 'r')
    for line in infile:
        ...
        if len(words) == 11:
            prob0, prob1, met, unmet, coverage, rmet, kmer = float(words[4]), \
                int(words[5]), int(words[6]), int(words[7]), int(words[8]), float(words[9]), words[10]
        else:
...
def main():
    ...
    parser.add_argument('-t', "--rtype", help="bismark, bedmethyl, freqtxt",
    ...
    if rtype == 'bismark':
        ...
        mposinfo = combine_fb_of_freqtxt(report_fp)
